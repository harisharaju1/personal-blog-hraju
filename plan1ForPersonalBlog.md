...
Since you're removing the UI write path, you have two options for adding posts:

**Option A — Supabase dashboard** (simplest): Insert rows directly into the `posts` table via the Supabase table editor.

**Option B — Keep an admin-only write route** (optional later): Add a `/admin/new` page protected by a hardcoded check (`if (user.email !== 'your@email.com') redirect('/')`) — no public registration needed.
...
- [ ] `pnpm build` succeeds
- [ ] Home page shows posts sorted by hot/top/new
